...
class MujocoGoalEnv(gymnasium_robotics.GoalEnv):
    """
    A custom gymnasium robotics goal-conditioned environment for reinforcement learning using ROS and MuJoCo.
    """
    ...
    def __init__(self, spawn_robot: bool = False, urdf_pkg_name: str = None, urdf_file_name: str = None,
        ...
        Args:
            ...
            unpause_pause_physics (bool): Whether to unpause and pause the simulation before and after each step.
        ...
        rospy.loginfo(self.CYAN + "Start init MujocoGoalEnv!" + self.ENDC)
        ...
        rospy.loginfo(self.CYAN + "End init MujocoGoalEnv" + self.ENDC)
    ...
    def _sample_box(self, box: gym.spaces.Box) -> np.ndarray:
        ...
        ``self.np_random``. See ``MujocoBaseEnv._sample_box`` for the full
    ...
    def _safe_unit_vector(self, vec: np.ndarray, eps: float = 1e-8) -> np.ndarray:
        ...
        ``MujocoBaseEnv._safe_unit_vector`` for the full rationale; this
    ...
    # ------------------------------------------
    #   Custom methods for the MujocoGoalEnv
