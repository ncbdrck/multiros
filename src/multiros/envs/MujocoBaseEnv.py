    ...
    def step(self, action: Any) -> Tuple[Any, float, bool, bool, Dict[str, Any]]:
        ...
        if self.sim_step_mode == 1:
            if self.unpause_pause_physics:
                ...
                # ``rospy.Timer`` at ``environment_loop_rate`` to refresh obs/reward/done from
                # the latest sensor values — this matches the real-env timing and is what
                # enables sim->real transfer. Like the Gazebo backend, MuJoCo does not enforce
